#!/usr/bin/env python3
"""FABLE/OWNER AUDIT REMEDIATION A5: curate a production-representative
cruise mock pool from the real, locked simulation books, matching the
existing base pool's category structure in frontend/src/lib/mock/sample_rounds.json.

Selection is WEIGHT-aware, not uniform-random over the raw books file: the
raw books pool over-represents rare outcomes (trigger books are 6.1% of the
raw pool here, vs the ~0.54% true shipped trigger rate) because the lookup
table's weight column - not raw book frequency - determines how often each
book is actually served. Within each payout/trigger bucket, this picks the
...
Run: scripts/assets/.venv/bin/python3 tools/mock/curate_cruise_pool.py
"""
import json
import subprocess
...
BOOKS_ZST = ROOT / "games/future_spinner/library/publish_files/books_cruise.jsonl.zst"
LOOKUP_CSV = ROOT / "games/future_spinner/library/publish_files/lookUpTable_cruise_0.csv"
...
def main():
    ...
    def add(category, pool, n=1):
        if not pool:
            print(f"WARNING: no candidates for cruise/{category}, skipping")
            return
        pool_sorted = sorted(pool, key=lambda b: weights.get(b["id"], 0), reverse=True)
        for b in pool_sorted[:n]:
            entries.append({"mode": "cruise", "category": category, "round": b})

    add("cruise_loss", loss, n=2)
    add("cruise_win_small", win_small, n=3)
    add("cruise_win_mid", win_mid, n=2)
    add("cruise_win_large", win_large, n=2)
    ...
    assert len(trigger_entries) >= 1, "cruise pool has zero natural Overdrive trigger rounds"

    with open(SAMPLE_JSON) as f:
        existing = json.load(f)
    existing_cruise = [e for e in existing if e["mode"] == "cruise"]
    print(f"existing cruise entries before curation: {len(existing_cruise)}")

    merged = [e for e in existing if e["mode"] != "cruise"] + entries
    with open(SAMPLE_JSON, "w") as f:
        json.dump(merged, f, indent=2)

    print(f"wrote {len(entries)} cruise entries to {SAMPLE_JSON}")
    for e in entries:
