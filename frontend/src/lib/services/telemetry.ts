...
export interface TelemetryBase {
  ...
  mode: string // bet mode in play (base/cruise/antelite/bonus/super)
...
// Win-tier thresholds as multiples of TOTAL BET. Aligned to the shipped C1 win
// celebration thresholds (WinBanner.svelte: BIG >=10x, MEGA >=30x, EPIC >=100x,
// MAX = the 5,000x cap) so an analytics "mega" event always matches what the
// player actually saw on screen - do not drift these two definitions apart.
