...
function judge(readings) {
  ...
  for (const r of readings) {
    ...
    // 4. THE SLAB CENTRES ON THE CANVAS. Asserted from R071 TASK 6, where the
    //    owner ruled the row's contents align their midpoint to the canvas
    //    centre. Until that ruling this was PRINTED rather than asserted,
    //    because it could not be satisfied without moving locked controls and
    //    that was not a builder's call. The ruling exists, the controls moved
    //    28px left as one translation of the row's origin, and the number is
    //    now a verdict.
    const off = +(r.slab.centreX - r.canvas.centreX).toFixed(2)
    ok(Math.abs(off) <= EPS,
      `${v}: the slab centres on the canvas centre (off by ${off.toFixed(2)})`)

    // And the same statement about the CONTENTS the slab backs, which is the
    // form the owner's ruling was written in.
    const contentsMid = +((c.max.left + c.steppers.right) / 2).toFixed(2)
    ok(Math.abs(contentsMid - r.canvas.centreX) <= EPS,
      `${v}: the row's contents centre their midpoint on the canvas centre `
      + `(midpoint ${contentsMid.toFixed(2)}, canvas ${r.canvas.centreX.toFixed(2)})`)
  }
  return failures
}
