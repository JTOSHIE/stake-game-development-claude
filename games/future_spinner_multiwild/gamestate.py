...
class GameState(GameCalculation):
    ...
    def assign_special_sym_function(self) -> None:
        """MULTIWILD variant: Wilds get a random multiplier when they land in the
        free game. Combined with the Overdrive meter via the 'combined' strategy."""
        self.special_symbol_functions = {"W": [self.assign_wild_multiplier]}

    def assign_wild_multiplier(self, symbol) -> None:
        """Assign a random multiplier to a Wild, free spins only (base Wilds plain)."""
        if self.gametype != self.config.basegame_type:
            symbol.assign_attribute(
                {"multiplier": get_random_outcome(self.config.wild_mult_values)}
            )
