...
class GameConfig(Config):
    ...
    def __init__(self):
        ...
        # -- Identity ----------------------------------------------------------
        self.game_id = "future_spinner_super"
        self.provider_name = "We Roll Spinners"
        self.game_name = "Future Spinner Super"
        ...
        # -- Bet modes ---------------------------------------------------------
        self.bet_modes = [
            # BASE MODE (cost 1.0x) - parity anchor -----------------------------
            BetMode(
                name="base",
                cost=1.0,
                rtp=self.rtp,
            ...
            # BONUS MODE / BUY (cost 100.0x) - reference buy ---------------------
            BetMode(
                name="bonus",
                cost=100.0,
                rtp=self.rtp,
            ...
            # SUPER BUY (cost 400.0x) - the new prototype -----------------------
            # Guaranteed 3+ trigger (standard bonus conditions), but the Overdrive
            # meter is pre-revved to 5x at the feature start (see gamestate), so
            # every free-spin win is multiplied from the first spin. The richness
            # comes from the pre-rev, not from extra scatters. Target 96.35% RTP
            # at the 400x price under the 5,000x cap.
            BetMode(
                name="super",
                cost=400.0,
                rtp=self.rtp,
