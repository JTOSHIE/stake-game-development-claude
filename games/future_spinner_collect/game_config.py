...
class GameConfig(Config):
    ...
    def __init__(self):
        ...
        # -- Identity ----------------------------------------------------------
        self.game_id = "future_spinner_collect"
        ...
        #
        # COLLECT variant: the low symbol L3 is ALSO the "collect coin". Marking
        # L3 as a special-symbol category lets draw_board() track its board
        # positions in special_syms_on_board["collect"] and lets the create-time
        # callback tag each free-spin L3 with a random cash value ("prize"
        # attribute). L3 keeps its ordinary ways paytable entry unchanged (the
        # ways engine iterates every board symbol and only checks the paytable),
        # so it pays ways wins AND carries a collect value at the same time. The
        # coin value is NOT a symbol multiplier (no "multiplier" attribute), so
        # the "global" Overdrive ways strategy is unaffected.
        ...
        # -- Collection-meter coin values (COLLECT variant) --------------------
        # When an L3 lands DURING FREE SPINS it becomes a collect coin carrying a
        # random value (a multiple of TOTAL BET). Values accumulate into an
        # in-round pot that is paid at the END of the free-spin round and reset
        # every round (stateless). The distribution is dominated by 0 ("blank"
        # coin, no value) because L3 is a common reel symbol (~21.5% of each
        # strip, ~4.3 per board); a heavy zero weight keeps the aggregate pot
        # modest so the freegame fence still converges to the 96.35% target.
        # A value of 0 assigns no "prize" attribute (the L3 stays a plain paying
        # symbol). Coin values are NOT scaled by the Overdrive meter.
        self.collect_values = {
            0: 84,   # blank L3 - no collect value (plain paying symbol)
            1: 8,    # 1x total bet
            2: 4,    # 2x total bet
            5: 2,    # 5x total bet
            15: 1,   # 15x total bet
            50: 1,   # 50x total bet (rare big coin)
        }
        ...
        # COLLECT fork ships only the two validated modes. Trim to {base, bonus}
        # so generate_configs / the optimiser operate on the kept subset only.
        _COLLECT_MODES = {"base", "bonus"}
        self.bet_modes = [bm for bm in self.bet_modes if bm.get_name() in _COLLECT_MODES]
