...
class OptimizationSetup:
    ...
    def __init__(self, game_config):
        ...
        # Only base + bonus survive the config trim; the opt_params table below
        # still references trimmed modes, so fall back to the global wincap for
        # any name not present before filtering the table to the kept modes.
        wincaps = defaultdict(
        ...
        # Keep only the fences for the modes that survived the config trim.
        _keep = {bm.get_name() for bm in game_config.bet_modes}
        self.game_config.opt_params = {
            k: v for k, v in self.game_config.opt_params.items() if k in _keep
        }
