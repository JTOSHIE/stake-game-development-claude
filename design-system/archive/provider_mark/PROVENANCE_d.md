...
> **SUPERSEDED 2026-07-26, non-preferred (convention (h)).** The owner has since
> commissioned a purpose-made mark externally, which is candidate **e**. Candidate d
> is kept, not deleted, and remains in the comparison sheet; it is also still the mark
> currently sitting in `design-system/brand/delivery/WeRollSpinners-Logo.png`, which is
> deliberately left unchanged until the owner confirms. See `README.md` for status.
