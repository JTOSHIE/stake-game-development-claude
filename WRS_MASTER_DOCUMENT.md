...
| 3 | **Automated bet-level verification limits** per star tier: maximum exposure, maximum payout multiplier, maximum bet cost, maximum cost multiplier, base SD floor and ceiling, P(>=5000x), P(>=10000x), CVaR, and ETL at two thresholds | See `COMPLIANCE_WATCH.md` 2026-07-25 section for the full table, both tiers | Every determinable constraint passes on **both** tiers, most by wide margins. Recomputed by a SECOND INDEPENDENT IMPLEMENTATION over the SAME shipped tables in `reports/qa/math_bet_level_compliance_2026-07-25.md`, which per convention (l.4) confirms the arithmetic and not the tables; the independent input is the books, and TR-110 is open against that chain's provenance | **PASS**, with two proximity flags and one open definition question, below. |
...
| Audio provenance | reports/audio/GENERATION_LOG_2026-07-13.md; sounds/README.md | DONE (JOB 1 merged to main 2026-07-14). **R146 (2026-09-25): OPEN for the owner's stems.** The DONE above now holds only for the July Stable Audio 3 set, which among the shipped files is ui_click.mp3 alone. The owner's stems supply every other shipped audio file and sounds/README.md records the R146 set, but the stems' source and licence were not stated in the drop and sit with the owner as an open question. |
...
| Provider logo (square, transparent) | **DONE 2026-07-26, pending only the owner's upload.** The delivery is `design-system/archive/delivery/WeRollSpinners-Logo.png`, candidate **f** at its native 1024, which carries a real alpha channel so the "transparent" requirement is met by the artwork rather than by a waiver. This supersedes the earlier hero-emblem and `brand_mark.svg` positions recorded on this row, and the alpha caveat they carried is closed. Candidate **g**, an owner-supplied 25-file variant pack that arrived the same day, was TESTED against f rather than filed beside it: f took 3 of 3 legibility measures at 32px and 2 of 3 at every other size on the ladder, so the delivered file did not change (`design-system/archive/provider_mark/PROVIDER_LOGO_DERIVATION.md`). g is superseded for the portal mark and adopted as the **studio brand set** for favicon, site and print, kept in whole at `design-system/archive/provider_mark/pack_g/`. |
...
| Round-two audio slots (bonus_trigger, buy_confirm, wild_land, coin_count, win_max, ambience_rain) | reports/archive/2026-07-14_job8-audio-round2-placeholder.md | OPEN, deliberately deferred (JOB 8) - gated on the owner playing the JOB 1 build and Fable ruling on the mix. Not started, by design. **R146 (2026-09-25): win_max is no longer deferred.** It ships from the owner's stems and plays on a spun max win and in Bet Replay; since R147 (2026-09-26) a bought max win plays it too, at its reveal (`frontend/src/App.svelte` buy path, commit b721febb; `docs/audio/AUDIO_TRUTH_MAP.md` section 4.5). The other five slots are unchanged. |
...
| 5 | **Model policy.** Sonnet High for mechanical and suite work; Opus for judgement; xHigh and above reserved for a single hard bounded problem in a short surgical session. The twice-failed escalation rule stands (`CLAUDE_PROJECT_INSTRUCTIONS_v9.md`: a brief failing its gates twice escalates one tier). |
...
**Rule 15, every work order carries a budget and the session is graded against its own
plan** (owner's order, 2026-07-29, transcribed from Fable via
`reports/briefs/FS_SESSION1_ADDENDUM_Prompt.md`). Every work order carries the budget
header from `reports/briefs/_TEMPLATE.md`; the session posts a Plan of Record before its
first expensive spend, including the verification cost computed at launch rather than
discovered afterwards, and is graded against it at close; priority and the degradation
order belong to the owner while the arithmetic belongs to the session, and the two do not
compute the same sum. Two invariants ride with it regardless of model: container
orchestration always, and the sight gate universal. The measured reason is at
`docs/skills/AGENT_BUDGET_AND_SCHEDULING.md` section 10: the session that produced the
...
## 9. CHANGE LOG
- 2026-07-29 (MULTI-TRACK PROTOCOL, rule 15): every work order carries the budget header from `reports/briefs/_TEMPLATE.md`, the session posts a Plan of Record before its first expensive spend and is graded against it at close, priority and the degradation order belong to the owner while the arithmetic belongs to the session. Container orchestration always and the sight gate universal ride with it. Transcribed from Fable per `reports/briefs/FS_SESSION1_ADDENDUM_Prompt.md`; authoritative text in `CLAUDE.md`.
...
- 2026-09-26 (R147): section 3's round-two audio slots row updated in place. A bought max win now plays win_max at its reveal as well (`frontend/src/App.svelte` buy path, commit b721febb), closing the bought-versus-spun inconsistency R146 owner-parked (`docs/audio/AUDIO_TRUTH_MAP.md` section 4.5).
