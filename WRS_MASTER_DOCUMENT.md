...
| 5 | **Model policy.** Sonnet High for mechanical and suite work; Opus for judgement; xHigh and above reserved for a single hard bounded problem in a short surgical session. The twice-failed escalation rule stands (`CLAUDE_PROJECT_INSTRUCTIONS_v8.md`: a brief failing its gates twice escalates one tier). |
