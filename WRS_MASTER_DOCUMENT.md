...
| Round-two audio slots (bonus_trigger, buy_confirm, wild_land, coin_count, win_max, ambience_rain) | reports/archive/2026-07-14_job8-audio-round2-placeholder.md | OPEN, deliberately deferred (JOB 8) - gated on the owner playing the JOB 1 build and Fable ruling on the mix. Not started, by design. **R146 (2026-09-25): win_max is no longer deferred.** It ships from the owner's stems and plays on a spun max win and in Bet Replay; a bought max win still plays win_epic plus echo, owner-parked (`docs/audio/AUDIO_TRUTH_MAP.md` section 4.5). The other five slots are unchanged. |
