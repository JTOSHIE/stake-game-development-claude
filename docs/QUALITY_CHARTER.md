...
| B2 | **Publisher name is set as type, not as a logo drop** | Lokis Vault renders `VALKYRIE` as letterspaced capitals on the tile (`published-tile-geometry.md:58`). A supplied logo image is used elsewhere, not composited into the tile. | MET. Recorded in `design-system/archive/provider_mark/PROVIDER_LOGO_DERIVATION.md`, which builds its size ladder from labelled anchors rather than an unobserved rendered size. |
...
| Q-07 | font-leak | `∞` at `HudOverlay.svelte:94` and the three infinite-autoplay buttons. U+221E is absent from the subset, so it does fall back. | **REVIEWED AND KEPT**, with the reason in the gate's allowlist. **COUNT CORRECTED 2026-08-15 by R071 TASK 8: it is now SEVEN instances, not four.** The allowlist entry and its reasoning are intact, VERIFIED at HEAD by direct read; what moved is the source, because each of the three layout profiles gained a pending state carrying the same glyph. **The disposition is unchanged and the number was never the disposition**, which is exactly why a count written into a row goes stale while the ruling behind it does not (convention (s)) |
...
| Q-11 | currency-format | The replay win pod (line 6 of the since-deleted WinPod component; removed 2026-08-13 by the R058 owner ruling, TR-137) rendered player money as `$winAmount.toFixed(2)`, the only money `.toFixed` left in `frontend/src`. Already carried as ledger row SA-022 from the analyst track. | **FIXED**, then the surface itself removed |
...
| Q-16 | hardcoded-string | Player-visible English not routed through the translation function, invisible to `locale_completeness_check.mjs` because that gate scans ALL-CAPS literals only. Counted rather than estimated: **27 static player-facing attributes and 48 markup text nodes**, listed in full in 4.3. TR-059 estimated "roughly thirty keys times sixteen locales"; the count confirms it. | **OWNER-PARKED, EXTRACTED; THE PARK IS RETIRED 2026-08-15 by R071 TASK 8.** The surgical pass this row was extracted for HAS LANDED and the park was never taken down, so the charter kept describing work as waiting that had already been done. VERIFIED at HEAD: a sweep for quoted alphabetic `aria-label`, `title`, `alt` and `placeholder` attributes across the component tree returns hits only on DEV-gated surfaces, which never render to a player. Section 4.3 below carried the same stale framing and is corrected with it |
...
| Q-24 | **format, pre-existing and larger** | **All eighteen `font-variant-numeric: tabular-nums` declarations in the codebase are inert**, for the same reason: Orbitron ships no `tnum`. That includes `.fs-num` on the **win count-up**, which the platform's guidelines require to count incrementally and which reviewers watch. This predates this session and was not introduced by it. | **CLOSED 2026-08-15 by R071 TASK 4, and closed at the CAUSE rather than per site.** This row's finding was that every `font-variant-numeric: tabular-nums` declaration in the codebase was INERT, because Orbitron ships no `tnum` feature for the property to switch on. That is no longer true of the shipped tree: **Exo 2 is now the money and counting face**, it carries real tabular figures, and the declaration does what it says. VERIFIED by measurement rather than by reading the CSS: Exo 2's digit spread at the 900 weight the money surfaces use goes from 19.30px to exactly 0.00 with the property applied. The per-digit box workaround TR-089 built for one site is RETIRED with it, VERIFIED by direct read (`WinBanner.svelte` carries no `.c1-digit` rule at HEAD). See TR-158 |
| Q-25 | **currency-format, incomplete** | **The Q-11 `WinPod` fix made its own overflow worse.** Routing through `formatBalance()` was correct for the format, but it ADDS a currency symbol and thousands separators, so the string is longer, in a box that is still 99px with `white-space: nowrap` and no fit action. Ledger row SA-022 named the overflow and the fix addressed only the formatting half. | **MOOT AND CLOSED 2026-08-15 by R071 TASK 8: the surface is gone.** There is no WinPod component at HEAD (the path is deliberately not backticked, because a backticked path is a claim that it exists), VERIFIED by direct read, and `winPrecision.test.ts` records the R058 TASK 2 deletion under the owner's design ruling of 2026-08-13. The replay view's end banner replaced it and is fitted by `autofitText`. **A row describing an overflow in a component that no longer exists cannot be actioned and should not read OPEN** |
| Q-26 | **format, incomplete sweep** | **The Q-12 `x` versus `×` fix was not swept to the class.** **CORRECTED 2026-07-29 by the boot-set audit, and the original row was wrong about the FILE as well as the count.** It read: *"Four more player-visible instances survive in `frontend/src/lib/config/fsModes.ts` blurbs and social blurbs: `1.6x`, `1.25x` twice, and `5x`."* `frontend/src/lib/config/fsModes.ts` contains none of those strings; `grep -n "1\.6x\|1\.25x" frontend/src/lib/config/fsModes.ts` returns nothing. The strings live in the i18n prose, and the recount is **51 player-visible instances, not four**: 6 in `frontend/src/lib/i18n/prose.ts` (`:90` and `:189` `modeOverboostBlurb` carry `1.6x` and `1.25x`, `:94` and `:192` `modeSuperBlurb` carry `5x`, in the real-money and social variants), and 45 across `frontend/src/lib/i18n/prose.locales.ts` (15 locales x `modeOverboostBlurb` twice plus `modeSuperBlurb` once). The same files already use `×` correctly 116 times, so this is drift within a convention the project has, not an unmade decision. **The fifth instance the original enumeration missed, in `WinBanner.svelte`, is FIXED**: its three remaining `10x`/`30x`/`100x` hits are comments at `:7`, `:50` and `:216`, none rendered. **Why it was wrong:** the instrument searched config and prose and never components, and it never enumerated the fifteen non-English locales at all, which is `FULL_AUDIT_METHOD.md` 2.5 exactly, a search that could not have returned the answer. **No gate covered this class** when this row was written: `frontend/scripts/machine_tell_gate.mjs` treats `×` as the correct glyph and does not assert the absence of the lowercase form. **One does now**, see the disposition. | **FIXED 2026-07-30 by the true fixdown, `fec8d61`.** All 51 instances swept to `×`, verified by `grep -c "1\.6x\|1\.25x\|5x"` returning **0** in both `prose.ts` and `prose.locales.ts` at HEAD. **The class is now held by `frontend/scripts/multiplication_sign_gate.mjs`**, seeded per convention (p) and wired into CI, so the sweep cannot silently reverse and a sixteenth locale cannot arrive carrying the lowercase form. **The re-sizing question this row put to the owner is MOOT and is recorded rather than deleted**, because the reasoning was sound and the row is the evidence for it: the sizing WAS wrong, the set WAS 51 across 16 locales rather than four English strings, and the correct response to a sizing that a recount contradicts is to re-derive it rather than to act on it. What made the sweep mechanical after all is that the glyph is invariant across every locale, so no translation judgement was involved. **This row was itself stale for the time between the fix and this correction**, which is the drift class `DOC_CURRENCY_GATE_SPEC.md` exists to hunt and which no gate yet catches, since a disposition cell reading OPEN beside a closed finding contains no dead path and no wrong count. |
| Q-27 | **scaffold-residue, incomplete sweep** | **The Q-13 fix took the font line out of the Vite scaffold block and left the rest of the block.** Still at HEAD in `frontend/src/app.css`: `color-scheme: light dark`, `background-color: #242424`, `a { color: #646cff }`, `a:hover { #535bf2 }`, and `body { display: flex; place-items: center }`. Every one is a starter default. | **MOSTLY CLOSED; the survivor list is corrected 2026-08-15 by R071 TASK 8.** Three of the five named survivors are GONE and the row still listed them: VERIFIED at HEAD, `frontend/src/app.css` carries no anchor rule at all, `#535bf2` returns zero hits across the frontend source and `index.html`, and `#646cff` likewise returns zero. **The row was listing scaffold residue that had already been swept**, which is worse than listing residue that remains: it invites a session to go looking for something and find nothing, then wonder which of the two records is wrong 
| Q-28 | **dist-hygiene, self-inflicted** | **The explanatory HTML comments added to `frontend/index.html` ship into `dist/index.html`.** Vite does not strip comments from the entry template. Mildly ironic given `dist_hygiene_gate.mjs` exists to stop documentation shipping. | **FIXED.** `frontend/vite.config.ts` `stripHtmlComments` removes HTML comments from the emitted `index.html`. `frontend/dist/index.html` at HEAD `90f21280` carries none. Source comments stay in `frontend/index.html`. |
...
| # | Class | Finding | Disposition |
|---|---|---|---|
| Q-30 | **hardcoded-string, and a gate blind spot** | **`LOADING CYBERNETICS...` rendered untranslated on the first screen of the game, in every locale**, and `locale_completeness_check.mjs` had been printing PASS over it. The gate only sees a literal that sits ALONE between tags; here the text shared its line with `{$assetLoadProgress}`, so the literal was never alone and the gate could not see it. **The Q-23 fix split them, and the gate found it in the same push.** So Q-23 did not introduce this defect, it exposed one. | **CLOSED ON BOTH HALVES 2026-08-15 by R071 TASK 8, and the charter said neither.** The string half was fixed; the GATE BLIND SPOT half, which this row correctly kept open, was closed by TR-091, which reads CLOSED in the review tracker. VERIFIED at HEAD by direct read: `locale_completeness_check.mjs` documents the escaped shapes it now handles and scans the `title` attribute among them. **The blind spot was the load-bearing half of this row and it is the half that had no record of being closed** |
...
**MATERIALLY CLOSED 2026-08-15 by R071 TASK 8, and this note is prepended rather than the
section rewritten, so the count below stays readable as the measurement it was.** The six
interpolation cases this section named now resolve through keys, VERIFIED at HEAD by
direct read of `frontend/src/lib/i18n/translations.ts`, which holds `buyFeaturesHeading`,
`betModesHeading` and `winFlash` with their social overrides beside them. **The COUNT below
is a dated measurement and is left exactly as it was taken**: it says what the tree held
when the blind spot was first quantified, which is the whole value of it. What was stale
was the STATUS, not the arithmetic.
...
**FIXED 2026-07-28 as TR-092, in the direction this section identified, and the section
never said so; corrected 2026-08-15 by R071 TASK 8.** VERIFIED at HEAD by direct read: the
property is removed at all three layout profiles in `HudOverlay.svelte` and the reasoning
is left in place at each site as a comment rather than the rule silently deleted, which is
the form this estate prefers because a future reader can see the decision instead of
inferring it from an absence. The refutation below is kept in full: **naming which of two
findings was WRONG is the more useful half of this section** and it does not go stale.
...
**CLOSED on the HUD by TR-092. CLOSED on the FEATURES chip 2026-08-15 (TR-152):** the chip
now reads `$tr` and no longer uppercases. Paytable and buy-dialog casing was not re-opened
in this pass.
...
**THE PARK IS RETIRED. Corrected 2026-08-15 by R071 TASK 8, and this note is prepended
rather than the section deleted, because the section's REASONING is still the best short
statement of why a hard bounded problem gets extracted instead of absorbed.** What is no
longer true is its STATUS. The surgical pass it was written for has happened. Checked entry
by entry at HEAD: of the ten static attributes it tabulates, none remains on a player
surface, and of the fifteen named markup strings, thirteen return zero hits anywhere in the
component tree. **This was the largest single stale surface in either the charter or the
tracker**, and it stayed stale for the ordinary reason: the pass that closed the work had no
instruction to come back and take down the sign that said the work was waiting.
...
**What was true on 2026-07-27, stated without softening then**: these strings render English
in all sixteen locales. They did before that session and they still did that day.

**CORRECTED 2026-08-15, stand-back.** The present-tense sentence above is no longer true of
the named HUD keys: `HudOverlay` now renders `$tr('hudSession')` and `$tr('stopOnWin')`.
The rest of the 4.3 starting set has not been re-counted in this pass and stays parked.
...
|---|---|---|
| **Audio** | **NEVER SWEPT.** Twelve shipped rows. No check on loudness consistency between rows, tail behaviour, the bed swap heard in context, or whether any row reads as generic. **R146 (2026-09-25): now 16 rows in 19 files, not twelve.** The owner's 15 stems replaced every shipped file except ui_click.mp3 and supplied the four cues that had no file: feature_enter, feature_end, retrigger and win_max. | The platform's own quality-rankings page warns by name against over-reliance on generic AI-generated assets, and every row here was model-generated. This is a scored axis with zero coverage. **R146 (2026-09-25): "every row here was model-generated" can no longer be asserted of the whole set.** Only ui_click.mp3 is still the July Stable Audio 3 file. The owner's 15 stems carry no stated provenance or licence in the drop: UNKNOWN, owner-parked. |
| **Social mode** | **CAPTURED AND GATED, 2026-08-15 recount.** `checks.yml` runs `social_dom_conformance`; committed frames and a social string gate exist. The 2026-07-27 "NEVER CAPTURED" sentence is the pre-gate description and must not be read as current. A full social-mode *quality* sweep of the vocabulary swap in context is still not the same thing as the conformance gate. | stake.us was BLOCKED on six visible prohibited-term strings at one point. The conformance surface now exists; a scored quality read of the swap is still a later wave. |
| **Accessibility beyond prohibited terms** | **COUNT CORRECTED 2026-08-15 by R071 TASK 8: there are TWO scripts, not one.** `a11y_social_proof.mjs` was added and proves the RENDERED accessibility tree carries the social wording at runtime, which is a stronger claim than the original's "no restricted phrase is hardcoded". **The load-bearing half of this row still stands and is not softened:** No focus order, no keyboard-only walk, no screen-reader pass. `contrast_gate.mjs` covers one label class, not all text. | Screen-reader text is player-facing text; that lesson already cost this project 14 mislabelled controls (R4/TR-012). The rest of the a11y surface has never been examined. |
| **Animation quality and timing** | **OVERTAKEN, corrected 2026-08-15 by R071 TASK 8.** "Nothing gates it" was true when written and is not true at HEAD: six CI legs now assert TIMED PRESENTATION BEHAVIOUR rather than frame rate, VERIFIED by direct read of `.github/workflows/checks.yml` (splash calm, turbo intensity, win count-up steady, win count-up sync, max-win hold and the retrigger moment). The distinction this row drew between rate and quality is the right one and it is what those legs assert: whether the presentation reads correctly over time, not how many frames it took. | The reviewer prompt names animation quality as one of the three axes real reviewers most often deduct on. |
...
| **The 560-value locale pass** | PARKED with a starting list at 4.3. Several named keys from that list (`hudSession`, `stopOnWin`) are now routed through `$tr`. A full recount of the remainder is not this pass. | The 2026-07-27 present-tense claim that the whole set still renders English is stale for the named HUD keys. |
