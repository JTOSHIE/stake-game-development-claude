...
|---|---|---|---|
| Q-01 | placeholder | `frontend/index.html:9` sets `<title>future-spinner-frontend</title>`, the Vite starter's npm package name. It ships verbatim to `dist/index.html`, and `grep -rn "document.title" frontend/src` returns nothing, so nothing overwrites it at runtime. This is the browser tab title on the live platform, in every session, in every locale. | **FIXED** |
