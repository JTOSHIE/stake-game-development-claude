...
| Q-27 | **scaffold-residue, incomplete sweep** | **The Q-13 fix took the font line out of the Vite scaffold block and left the rest of the block.** Still at HEAD in `frontend/src/app.css`: `color-scheme: light dark`, `background-color: #242424`, `a { color: #646cff }`, `a:hover { #535bf2 }`, and `body { display: flex; place-items: center }`. Every one is a starter default. | **PARTLY CLOSED 2026-07-31, and the parts are recorded separately because two sessions touched this row without either noticing the other.** The scaffold `button:focus` rule was replaced by `4e8cfb5` (S2-C017), and `body { display: flex; place-items: center }` was removed by `3047c61`. **The second was not cosmetic**: with `overflow: hidden` on the replay route, centring content taller than the viewport cut the compliance disclaimer off the top with no way to scroll to it, measured at 5 of 8 presets. The real fix was to let that route scroll; the residue removal came with it. **STILL OPEN after 2026-08-15**: `color-scheme: light dark` and `background-color: #242424`. The stock indigo `button:hover` is now the brand cyan. The `a { #646cff }` / `#535bf2` rules and `#app` max-width/padding/text-align were already gone when this pass counted.
