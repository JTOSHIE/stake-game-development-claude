...
| Q-26 | **format, incomplete sweep** | **The Q-12 `x` versus `×` fix was not swept to the class.** Four more player-visible instances survive in `frontend/src/lib/config/fsModes.ts` blurbs and social blurbs: `1.6x`, `1.25x` twice, and `5x`. | **OPEN.** Small and mechanical; belongs with the locale pass, since those same strings need translating. |
