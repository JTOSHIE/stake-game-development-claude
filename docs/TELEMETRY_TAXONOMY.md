...
max >= 5000, epic >= 100, mega >= 30, big >= 10, small > 0, else none. Aligned to the shipped
C1 win-celebration thresholds (WinBanner.svelte) so an analytics "mega" event always matches
what the player saw on screen. These are studio choices, not platform standards; adjust per
skin in `telemetry.ts` `WIN_TIERS` (and keep WinBanner.svelte in step if you do).
