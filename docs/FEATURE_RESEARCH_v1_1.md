# FEATURE RESEARCH v1.1: BET-MODE + MECHANIC DESIGN SPACE

> **Provenance note (added on recovery, 2026-07-05).** This is the ante and multi-buy
> feature research produced in an earlier session. It was committed to feature branches
> (`claude/gap-analysis`, then carried onto `claude/lumen-sideproject`) as
> `docs/MATH_DESIGN_SPACE.md` but never merged to `main`, so from `main`'s perspective it
