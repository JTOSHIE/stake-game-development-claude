...
| 5 | Assets and imagery not offensive or inappropriate | **PASS** | All art produced in-house from vector masters or owner-approved enhancement; no third-party or generated-likeness content. **AMENDED 2026-08-09. The sentence was true of the art we MADE and blind to a file we SHIPPED.** An 886KB image shipped in the bundle at the future-spinner theme's backgrounds folder, under the name bg-1.jpg, sha256 114c9e6c..., and nobody in this project had ever opened it: it appears in the record only as a SIZE, at REVIEW_TRACKER TR-112, rated low because it is not player-visible. It is a machine-generated cityscape carrying legible neon signage for RIVAL GAMBLING BRANDS, including JACKPOT CITY, a real online casino, plus CYBER JOKER SLOTS and NEON REELS, and garbled generated lettering such as OKERLOTS. **Not player-visible is the right measurement of the wrong thing**: the sibling published item is worded 'Game should not CONTAIN the Stake Engine Loader', where contain plainly means ships in the bundle, and the portal exposes a per-file listing a reviewer can open. It also put the word JACKPOT into the artefact, which the prohibited mechanic gate asserts is absent, and that gate reads text and cannot read pixels. **The owner confirmed the background is no longer used.** REMOVED: the file is deleted, the two references are repointed at the real backdrop, and every theme's background is now named bg_base.jpg. Bundle 15,719,656 to 14,833,185 bytes, exactly the 886,471 the file weighed. **THE GAP THIS LEAVES OPEN: no gate reads TEXT INSIDE A SHIPPED IMAGE.** |
| 6 | Sufficiently distinct from existing titles and series | **PASS** | Original IP, original mechanic naming (Overdrive Free Spins), studio marks only in shipped assets and text. **SCOPED 2026-08-21 by R078:** the sentence was true of every mark we CHOSE and became false at R076, when the platform's MANDATED General Disclaimer block entered shipped text carrying the platform's own closing mark. That block is required verbatim, so the mark is carried rather than adopted; everywhere outside it the rule is unchanged. |

## Game Thumbnail

| # | Item | Status | Evidence |
|---|---|---|---|
| 7 | Thumbnail meets Stake artwork guidelines | **PASS** | **CLOSED 2026-08-09 by the platform's own assertion, which is better evidence than a visual read.** The pre-submission checklist at the approval screen reports **"Thumbnail is set"** with a green tick. The Tile Editor separately shows the composed tile rendering: robot, FUTURE SPINNER, WE ROLL SPINNERS on the neon city background. **A note on what the platform actually stores:** the Tile Editor composes from SEPARATE layers, a background image, a foreground element, a gradient and a game title, rather than from our single pre-composed master. So `design-system/archive/delivery/FutureSpinner-Tile.png` at 408x546 is the reference and the record of geometry, not necessarily the bytes the portal holds. The old text said the portal still showed a Design Thumbnail placeholder in every capture; that predates the tile rework and is superseded. ORIGINAL FINDING, retained: | Composed in the Tile Editor from `03_branding/FutureSpinner-BG.jpg` and `-FG.png`. The portal still shows a "Design Thumbnail" placeholder in every capture, so this is **not yet done**. |
...
| 23 | Functions correctly on Popout S/L | **PASS** | Both captured. Popout L renders the full landscape HUD; Popout S renders the mini strip with all seven controls. **Updated 2026-08-10 (R043 PHASE 3a, fresh-context major 18): this row read "PASS with a defect" citing the mid-glyph WIN clip and the scrollbar, and both cited tracker rows have long been CLOSED**: TR-066 closed both halves (the `--autofit-scale` wiring fix, the measured fitMoney abbreviation and the five-state proof against the owner's own capture) and TR-065 closed with the layout re-derivation, `layout_fit_gate.mjs` green at all seven presets. The one Popout-S money item still open is the narrower TR-086 (sub-390px widths BELOW the platform preset), which has its own row and an owner escalation. |
| 24 | Functions correctly on Mobile | **PASS** | Mobile portrait and Mobile S captured and playable. **Updated 2026-08-10 (R043 PHASE 3a, fresh-context major 18): the "same scrollbar defect" clause referred to TR-065, CLOSED 2026-07-26** by re-deriving the layout maths (runtime-measured wordmark and HUD-slot heights), with `layout_fit_gate.mjs` asserting no scroll, no offscreen control and no clipped readout at all seven presets. |
...
| 32 | Check 5 wins for each game mode against the Game Rules | **OWNER/OBSERVE** | **The published maths half is now closed exhaustively, 2026-08-09.** `frontend/scripts/round_payout_reconciliation_gate.mjs` reconciles EVERY win in EVERY published book against the paytable the player reads on screen: 500,000 rounds, 3,618,404 wins, all five modes, zero disagreements. It parses the paytable twice from genuinely independent sources (a python dict in the locked maths package, a TypeScript array in `PaytableModal.svelte`) and checks they agree before trusting either. That is 25 hand-drawn samples replaced by the whole population. **What remains with the owner is the LIVE half**: that the RGS serves these published books faithfully, which no repository artefact can show. Five wins per mode on stake-engine confirms it. **One modelling fact came out of building it, CORRECTED 2026-08-10 (R043 PHASE 3a, fresh-context major 17): the 5,000x cap is applied at BOTH levels, each individual win is min(formula, 5,000x) AND the round total is min(sum of wins, 5,000x).** This cell previously said "PER WIN, not to the round total", which the gate it cites contradicts in its own header (`round_payout_reconciliation_gate.mjs:34`: "each win is individually min(formula, cap), and the round is min(sum of those, cap)") and which books_base round 1020 disproves directly: presented wins total 977,560 centibets, paid payoutMultiplier 500,000, the round capped (OWNER_RULINGS section A1). The shipped rules text states the per-round basis. The six-scatter question this pass also surfaced is CLOSED and was already closed: see the owner ruling recorded at TR-017a and restated 2026-08-09. |
...
| 46 | **English is the only supported language in Social Mode** | **PASS** | **THE FAIL VERDICT WAS WRONG, AND SO WAS THE RE-DERIVATION OF 2026-08-05 THAT UPHELD IT.** Both looked at `i18n/tr.ts`, saw `$locale` passed through untouched, and concluded nothing forces English. That is the wrong LAYER. `tr.ts` translates into whatever locale is set; the enforcement sits upstream at the locale store. Two routes, both in `frontend/src/lib/stores/socialLocale.ts`: its launch resolver checks social FIRST, so the URL route cannot produce a non-English social session before first paint; and its social-English enforcer, called from `frontend/src/App.svelte`, subscribes to the social store and sets the locale to English whenever social turns on, which covers the authenticate route where social arrives after paint. `REVIEW_TRACKER.md` TR-067 already reads CLOSED. **PROVEN EMPIRICALLY 2026-08-05 against a real build rather than by reading**: `?lang=de` renders German ("Spiel nicht verfuegbar"), while `?lang=de&social=true`, `?lang=fr&social=true` and `?lang=ja&social=true` all render English ("Game unavailable"), with the social vocabulary also applied (COINS rather than BALANCE). ORIGINAL FINDING, retained because the correction is the record: | See TR-067. `i18n/tr.ts:14` derives from `[locale, isSocial]` and uses `$locale` regardless: social mode switches the *vocabulary*, never the *language*. A social session launched with `lang=de` would render German. Nothing forces English. **RE-DERIVED 2026-08-05 AND STILL FAILING**: `tr.ts` still reads `derived([locale, isSocial], ([$locale, $social]) => t($locale, key, $social ? 'social' : 'real', params))`. The social flag still selects the VOCABULARY argument only, and `$locale` is still passed through untouched. **AMENDED 2026-08-09, and the PASS was right about the GAME route and blind to the REPLAY one.** App.svelte branches on its replay flag and renders ReplayMode INSTEAD of the game tree, so the boot-time call to resolveLaunchLocale never governed a replay. That call forces the locale STORE, and the replay route never read the store: parseReplayParams took lang raw from the URL, and ReplayMode's eager first-paint block read it raw a second time. **WHAT HID IT: a social replay with lang=de renders an ENGLISH disclaimer**, so anyone who opens the page and reads it concludes the rule holds. That one key survives because replayDisclaimer carries an explicit social override whose text is English. Keys with no override did not: hudStartReplay gave WIEDERHOLUNG STARTEN, replayAgain gave ERNEUT ABSPIELEN, and money rendered 1.234,50 SC. All three paint only AFTER a replay loads, so a browser probe with no RGS behind it never reaches them, and my own first probe showed no difference before and after the fix. **FIXED**: both reads routed through the same validator, in `frontend/src/lib/services/replayService.ts` and `frontend/src/lib/components/ReplayMode.svelte`. Pinned by `frontend/src/lib/services/replayLocale.test.ts`, which drives the parser rather than the resolver, because the resolver was never the broken part, and which was proven RED against the reverted line before being accepted. Now a CI step. **Symbol names are deliberately unbacketed here: the currency gate binds every backticked identifier in a row to that row's FIRST file:line citation, which for this row is a different file.**
