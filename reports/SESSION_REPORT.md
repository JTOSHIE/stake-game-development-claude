...
# Session Report - R074 THE ONE-SHOT SUBMISSION AUDIT (2026-08-20)

Brief saved verbatim: `reports/briefs/FS_FABLE_R074_FINAL_AUDIT_Prompt.md`, the sole
live brief. Branch: `main`, as integrator, per multi-track rule 1. Locked paths
untouched; `locked_paths_gate.mjs` PASS at every commit. Audit posture throughout:
evidence plus escalation; zero fixes landed on any shipped surface; no wallet call, no
portal mutation, nothing ticked, nothing submitted. The close crossed local midnight;
every record carries the brief's date.

## Summary

**The verdict is GO FOR START APPROVAL**, recorded with its evidence at
`docs/records/FINAL_SUBMISSION_AUDIT_2026-08-20.md`, beside the fifty-one walked at HEAD
in `docs/records/GUIDELINES_51_FINAL_AUDIT_2026-08-20.md`. The published `a95c521a`
build stands: a fresh build at the tip reproduces the published stamp byte for byte, 93
files, 12,467,624 bytes.

## What ran, phase by phase

**Phase 0, the anchor: HOLDS.** Tip `a95c521a` exactly, tree clean, no open PRs, newest
remote run green (32364167540, the one-commit-lag run R073 left). Fresh first-hand reads
of the whole anchor set. Every tracker row not CLOSED carried into the verdict with a
disposition.

**Phase 1, platform truth: ZERO DELTA.** All 64 /docs routes enumerated live and
captured to `docs/stake-engine-live/2026-08-20/`; every body byte-identical to
2026-08-15; the RGS trio unchanged since 2026-07-29; the submission-morning refresh
stands done. **The logged-in half is escalated whole (the E1 bundle)**: the pane's
portal session has expired and sign-in wants terms boxes plus an OAuth grant, which are
owner actions. The owner-quoted first-time-publisher sentence exists in no committed
capture and on no public route; the public framing page is byte-stable since 2026-08-10.

**Phase 2, the fifty-one: 38 CONFIRMED, 12 STALE-refreshed, 1 ESCALATED.** Nine
read-only verifier agents, adversarial refutation, first-hand spot-checks. No item's
substance failed; the twelve stale rows are mapping-cell drift from passes that landed
after 2026-08-13, corrected in the new table with the dated mapping left unedited.

**Phase 3, the estate: GREEN EVERYWHERE RUNNABLE.** The full CI battery re-run locally
from checks.yml's own command lines: **static 74 of 74, browser 28 of 28, seeded
self-tests included**, FS_WRITE_EVIDENCE unset, tree clean throughout. Remote truth: the
full matrix is 30 of 30 at `307989ad` and every later commit is record-only, verified by
diff. Local-only harnesses: currency_conformance PASS (the fifty-row),
locale_launch_conformance PASS 16 of 16 plus 10 of 10 fallbacks; **three local-only
harnesses are stale at HEAD** (modal_safety_proof, portrait_layout_conformance,
platform_conformance_item2), the R066 class with three live instances, escalated as G3.
The kit reproduces the published stamp byte for byte; dist ships only the future-spinner
theme, verified directly.

**Phase 4, the maths: EXACT.** RTP, cap and wincap frequency recomputed from the shipped
lookup tables with exact rational arithmetic, costs from the shipped index.json: every
figure reproduces the record to ten places in all five modes. **Books-to-lookup equality
re-run in FULL at HEAD: 500,000 rounds, 4,455,829 assertions, 0 failures**, all five
book hashes verified against BOOKS_MANIFEST.md before the run, which is the input-bound
provenance TR-110 asked for. The blurb-to-truth matrix verifies row by row against
primary sources. The portal Math page read rides E1.

**Phase 5, the gap hunt.** Four read-only lenses, 27 findings, every one adversarially
verified, 7 refuted. The en sweep and the sixteen-locale integrity sweep came back clean
on typos, placeholders, register, keysets, slots and mojibake. The survivors are gaps G1
to G9 in the verdict record, all escalated with evidence: the two submission-material
items are the dossier 5g checklist fetch now blind behind the login wall, and the
hardcoded English speed tooltip with internal drift; the quality items include the stale
harness class, the popout 44-versus-40 label (the R073 open thread, restated), the
ellipsis glyph split, two social lines reading machine-substituted, the twin win-tier
vocabularies, and the untracked OFL licence texts; the record-only set is enumerated for
one records pass. Parts (a) and (d) ride E1.

## Verification

Local at the tip, this session: the complete static suite and browser matrix as above;
doc_currency PASS at 272 frozen, 0 new, at every commit; dash and machine-tell scans
clean over source and dist; every commit staged explicit paths and every gate invocation
chained with `&&` per (u.1).

## The remote runs, recorded per rule 10

Every push this session went green: 32372996744 (`50aeffae`, the brief), 32373522535
(`3cbef98c`, the capture set), plus the runs for `bc6f7475` (the fifty-one table) and
`cc56e248` (the verdict record), verified green before this report was committed. The
report commit itself carries the usual one-commit lag; its run is recorded by the next
session, and the final owner preview lands after it per rule 12.

## The owner preview, per rule 12

Refreshed before this report, line quoted as evidence:

OWNER PREVIEW  |  v10 line, main  |  commit cc56e248  |  built 2026-08-21T00:06:07+10:00  |  http://192.168.4.92:5173/

Run once more as the last action of the close, after the final push.

## Self-audit against the brief, per the facts discipline point 4

The brief's hard prohibitions all held: no wallet call, no portal mutation, no
placeholder evidence, no retuning, locked paths untouched. The fix policy admitted zero
fixes and zero were made; every finding is escalated with evidence. Two of the session's
own instruments failed before the estate did and both are recorded rather than smoothed
over: the first browser-battery attempt failed on the driver's own working directory,
not the gates (all 28 legs passed once the driver was corrected), and an early read of
the checklist page as freshly rewritten was corrected by file-to-file diff against the
dated captures before anything was recorded, the framing text having been stable since
2026-08-10. The one thing the brief asked that no session can do is the logged-in read
set; it is escalated as E1 rather than approximated.

## FOR THE NEXT SESSION

Nothing is queued, by the brief's own close: the owner reads the verdict with Fable,
walks the fifty-one with the final audit table beside the Guidelines tab ticking
nothing, performs the E1 logged-in glance on his own machine, and presses Start Approval
on his own word only. Model and effort: Fable, maximal depth, one session, integrator on
`main` throughout.
