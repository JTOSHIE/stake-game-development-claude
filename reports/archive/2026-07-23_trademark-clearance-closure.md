...
## ADDENDUM: TRADEMARK CLEARANCE CLOSURE, 2026-07-23

Closes both threads left open by the addendum above. **The submission gate
on names is now closed** - `WRS_MASTER_DOCUMENT.md` section 1's trademark
clearance row moves from `IN PROGRESS` to `DONE`.

A same-day automated USPTO retry was attempted first (a longer cooldown had
passed since the 2026-07-18 CloudFront block, and reconnaissance did
initially succeed - confirming the search field is an Angular Material
`mat-select`, that quoted phrases do not restrict the plain Wordmark search
to exact-phrase matching, and that a "Field tag and Search builder" mode
exists but was untested), but the site re-blocked with the same 403 after
only ~5 requests - lower tolerance than the previous block, not higher.
Rather than keep retrying into what was clearly a low, consistent
automation threshold, the owner was asked to choose a path and opted to run
the three USPTO searches manually.

**Fable's AU similarity ruling (both names CLEAR):** reviewed against the
full 2026-07-18 variant-scan dataset (19 hits). "Future Spinner": zero
results for "future spin"; nearest mark is SPINNER SLOTS (number 1876622 -
note the brief's own text quoted this as "2126622", corrected here against
the actual captured record rather than propagating the transposition -
MURKA GAMES LIMITED, class 9 social slots software), sharing only the
descriptive "spinner" element common across gaming marks, distinctive
elements and overall impressions differ. "We Roll Spinners": nearest marks
("That's how we roll", the SunCoast Spinners sporting marks, the
venue-limited "Spinners" bar mark) all sit in unrelated services with
distinct overall impressions. No barrier identified either way.

**USPTO, owner-conducted:** the owner manually ran all three searches ("We
Roll Spinners", "Future Spinner", "future spin"; live marks; classes 9/41,
Coordinated broadening off) on the live USPTO system and attests all three
clear, dated 2026-07-23. No register data was reproduced since none was
captured by automation; if screenshots are supplied later they land under
`docs/records/trademark/2026-07-23/uspto-owner/`, non-blocking.

### Verification

- `git diff --stat -- WRS_MASTER_DOCUMENT.md` before this addendum's edit:
  confirmed clean (untouched by the 2026-07-18 pass, as recorded there);
  this addendum makes the one, deliberate row-status edit plus a section 9
  change-log line, with the exact caveat text from the brief retained
  verbatim in the row's notes.
- `docs/records/trademark/2026-07-15/SEARCH_LOG.md`: the 2026-07-15 and
  2026-07-18 entries are unchanged in substance; a new `## 2026-07-23`
  section is appended below.
- Locked files confirmed untouched (`rgsService.ts`, `gameStore.ts`,
  `games/future_spinner/`, `.claude/settings.json`).

### Not done (by design)

- No new automated USPTO register data was gathered this pass - the
  closure rests on the owner's manual attestation, exactly as the brief
  specified, not on a fresh automated capture.
- Owner-supplied USPTO screenshots are still pending; referenced as
  non-blocking future evidence in the SEARCH_LOG entry, not required to
  close this item.

### FOR THE NEXT SESSION

- **Model/effort:** Claude Sonnet 5, default effort. **Approach:** treated
  this strictly as a closure/recording task - no new similarity judgment of
  this session's own was introduced anywhere; Fable's ruling and the
  owner's attestation were transcribed as given, with one factual
  correction (the SPINNER SLOTS mark number, verified against the actual
  captured JSON rather than the brief's inline text) rather than silently
  reproducing a transposition into a permanent compliance record.
- **Alternatives rejected:** none - this was a direct transcription/closure
  task with a single narrow judgment call (correcting the mark number
  against source data) rather than a task with multiple viable approaches.
- **Files touched:** `docs/records/trademark/2026-07-15/SEARCH_LOG.md` (new
  `## 2026-07-23` section appended, prior entries unchanged);
  `WRS_MASTER_DOCUMENT.md` (section 1 trademark row IN PROGRESS -> DONE,
  section 9 change-log line added); this report. Locked files untouched.
- **Open threads:** the submission gate on names is closed. Owner-supplied
  USPTO screenshots remain a nice-to-have, non-blocking addition if they
  arrive later. No other open trademark work.
