...
| **Q-27** | Vite scaffold CSS remnants in `app.css`. | **OPEN and WIDER than recorded.** Session 1 added a player-visible instance the row did not have: `app.css:160-162` styles `button:focus` with Chrome's own `-webkit-focus-ring-color` `rgb(0, 95, 204)`, so a stock browser blue ring sits on the last-touched control through spin, win and COLLECT on a cyan and magenta game. Also still open: stock indigo link colours, `background-color: #242424`, scaffold body centring, and `#app { text-align: center }` which strands paytable bullet markers (cluster C-07, the one cluster both panel seats CONFIRMED outright). **RECOUNTED 2026-07-31 and NARROWER than this row, not wider.** Both instances named here are CLOSED at HEAD. The scaffold `button:focus` rule was replaced by `4e8cfb5` (S2-C017) and the stock `rgb(0, 95, 204)` survives only inside the comment explaining that fix. The body centring was removed by `3047c61` and `place-items` likewise survives only in the comment recording why. **That second removal was not cosmetic**: with `overflow: hidden` on the replay route, centring content taller than the viewport cut the compliance disclaimer off the top with no way to scroll to it. What genuinely remains of Q-27 is the link colours, `color-scheme`, the `#242424` background and `#app`'s `max-width`, `padding` and `text-align`, which the charter row now carries. |
| **Q-28** | Explanatory HTML comments shipping in `dist/index.html`. Not frame-visible. | OPEN, small. |
| **Q-34** | The same mode reads `Cruise` on the features menu, paytable and buy dialog and `CRUISE` on the HUD badge, via `text-transform: uppercase` present on one surface class and absent on three. | **CLOSED on the HUD (TR-092).** The FEATURES chip leftover was a second instance and is FIXED 2026-08-15: the chip now reads `$tr` and no longer uppercases. |
