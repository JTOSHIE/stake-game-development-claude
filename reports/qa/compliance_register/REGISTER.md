...
| REQ-004 | ARTEFACT | YES | `approval_guidelines.md:26` | No Stake marks, logos or Stake-themed motifs anywhere in shipped art, audio or text. **SCOPED BY REQ-016, R078 2026-08-21:** read against `general-disclaimer.md:18` from the same corpus, this governs everything the studio CHOOSES to ship. The platform's own mandated General Disclaimer block is the one exception, required verbatim and carrying the platform's closing mark by mandate. See REQ-016 for the re-reasoning and the evidence. | 1 | R1-08 |
...
| REQ-016 | ARTEFACT | YES | `approval_guidelines_general_disclaimer.md:22` | **RE-REASONED 2026-08-21 BY FABLE RULING R078, on R076 and R077. THE CONCLUSION BELOW REPLACES THE 2026-07-29 ONE; THE METHOD THAT REACHED IT STANDS.** **The re-reasoned answer: the mandated block is SANCTIONED shipped text and the studio's own appended sentence is REMOVED.** The platform's words, quoted rather than characterised per convention (l.7), are unchanged from the 2026-07-29 reading. *The mandate sentence*, `approval_guidelines_general_disclaimer.md:18`: "The game rules/information popup must include a brief disclaimer regarding game operation." *The permission sentence*, same line: "You are able to use our template disclaimer, or your own, so long as the same message is clearly conveyed." *The template's closing line*, `:22`: "TM and © 2026 Stake Engine." *The platform's own asset restriction*, `approval_guidelines.md:26`: "Game assets cannot include material with Stake™ branding or themes." **WHAT CHANGED IS NOT THE TEXT BUT THE EVIDENCE, and that is the whole lesson of this row.** The 2026-07-29 reasoning inferred from the permission sentence that the template was optional, therefore the closing line was not mandated, therefore this project's studio-marks-only rule continued to govern, therefore we should ship our own wording and our own marks. Every step followed from the corpus available. It lost to two surfaces that corpus does not contain. **R076:** the owner reached Step 1 of 4 of the Start Approval form, which is where the requirement is actually enforced, and the mandated text is what it wants; the estate's professionally translated paraphrase, correct on substance, lost on the letter. **R077:** the owner captured a LIVE title's disclaimer in production and it ships the mandated paragraph with NOTHING appended, which overruled the one studio sentence R076 had allowed beside it. **A production capture and a submission form outrank a reading of the documentation**, and neither was reachable from the mirror. **SHIPPED POSITION, verified 2026-08-21 by direct read at the R078 tree:** `frontend/src/lib/i18n/disclaimer.ts` holds the block as ONE constant, 472 characters, byte-identical to `general-disclaimer.md:18` and to all five dated captures, in which it is the LAST line with nothing following it; all sixteen locales and both modes resolve to it; `frontend/src/lib/components/PaytableModal.svelte` renders it as a bare read with nothing appended at the render site. **CONSEQUENCE FOR REQ-004 AND FOR CLAUDE.md's STANDING RULE:** the studio-marks-only rule is not overridden, it is SCOPED. The mandated block is its one exception, the mark inside it is carried by mandate rather than adopted, and outside that block the rule governs unchanged and is enforced by `frontend/src/lib/i18n/disclaimer_conformance.test.ts`. **THE PARKED OWNER QUESTION IS CLOSED:** it asked whether the platform EXPECTS the attribution despite not requiring it in prose. The form and the live titles both answer yes, so nothing is left to interpretation here. | 1 | R1-26 |
