...
**CORRECTION, 2026-07-31.** S2-C060's recorded derivation contains a false statement and it must be settled before any session acts on the row. It claims the dated mirror holds ZERO occurrences of end-round; six files under the 2026-07-29 capture contain them, including `rgs.md:37`. The mirror predates the derivation, so the grep was wrong when written. The platform text makes the finding STRONGER, not weaker. Recorded additively in `waveA_raw.json` under `correction_2026_07_31` rather than by rewriting the wave-A text.
