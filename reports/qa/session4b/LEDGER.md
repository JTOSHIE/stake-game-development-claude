...
|---|---|---|
| **S2-C115** | MEDIUM | **ONE LINE.** Replace `rgsService.ts:525` `const lang = p.get('lang') ?? 'en'` with `const lang = resolveLaunchLocale(p.get('lang'), socialAtBoot, locales)`, importing the existing non-locked authority from `../stores/socialLocale`. The wire value and the screen value become the same function of the URL. No import cycle: `socialLocale.ts` imports `gameStore`, `socialMode` and `i18n/translations`, none of which imports `rgsService`. |
| **S2-C061** | HIGH | **ONE LINE plus two non-locked files.** Add, immediately after `rgsService.ts:735`, `rgsBetLimits.set({ min: auth.minBet, max: auth.maxBet, step: auth.stepBet, default: auth.defaultBetLevel })`, mirroring the already-sanctioned bet-levels passthrough exactly. The new store `frontend/src/lib/stores/rgsBetLimits.ts` and the guard in `betLadder.ts` are NOT locked. |
